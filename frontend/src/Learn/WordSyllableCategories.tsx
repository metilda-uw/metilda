...
class WordSyllableCategories extends React.Component<Props, State> {
    ...
    async componentDidMount() {
        ...
        if (!isNaN(initNumSyllables)) {
            wordSyllableService.getWordsBySyllableCount(initNumSyllables)
                .then((res) => {
                    this.setState({
                        imageSrcList: res.data.map((word: any) => word.imagePath)
    ...
    componentDidUpdate(prevProps: Readonly<Props>, prevState: Readonly<State>, snapshot?: any) {
        ...
        const currentValues: queryString.ParsedQuery<string> = queryString.parse(this.props.location.search);
        const currentNumSyllables: number = parseFloat(currentValues.numSyllables as string);

        const prevValues: queryString.ParsedQuery<string> = queryString.parse(prevProps.location.search);
        const prevNumSyllables: number = parseFloat(prevValues.numSyllables as string);
        ...
        if (currentNumSyllables !== prevNumSyllables) {
            ...
            wordSyllableService.getWordsBySyllableCount(currentNumSyllables)
                .then((res) => {
                    this.setState({
                        imageSrcList: res.data.map((word: any) => word.imagePath)
                            .filter((word: any) => word !== null),
    ...
    resetSize = () => {
        if (this.state.imagesLoaded) {
            for (let i = 0; i < this.imageSrcList().length; i++) {
                ...
                const pitchArtElement: HTMLElement = document.getElementById("pitchArt" + this.getNumSyllables() + i)!;
    ...
    render() {
        ...
        if (this.state.imagesLoaded) {
            imageContent = <div className="row">
