...
# For Role based question mapping and other relevant operations
#---------------------------\\------------------------------------#

# Add role to question
@app.route('/api/addRoleToQuestion', methods=["POST"])
def addRoleToQuestion():
    with Postgres() as conn:
        postgres_insert_query = """ 
        INSERT INTO questionRoleMapping (qid, userrole, userid, createdat)
        ...
        mapping_id = conn.execute_insert_query(postgres_insert_query, record_to_insert)

    # if query does not return mapping_id, return error
    if mapping_id is None:
        return Response("{'result': 'query unsuccessful'}", status=500, mimetype='application/json')

    return jsonify({'result': mapping_id})


#remove user role from mapping to a question
@app.route('/api/removeRoleFromQuestion/<int:qid>/<string:userrole>', methods=["DELETE"])
def remove_role_from_question(qid, userrole):
    try:
        with Postgres() as conn:
            postgres_delete_query = """ 
            DELETE FROM questionRoleMapping
            WHERE qid = %s AND userrole = %s
            """
            result = conn.execute_update_query(postgres_delete_query, (qid, userrole))
...
# Get Questions by User Role
@app.route('/api/getQuestionsByRole/<userrole>', methods=["GET"])
def get_questions_by_role(userrole):
    try:
        with Postgres() as conn:
            query = """
            SELECT q.*
            FROM QUESTIONS q
            JOIN questionRoleMapping qrm ON q.qid = qrm.qid
            WHERE qrm.userrole = %s AND q.isactive = TRUE
            """
            results = conn.execute_select_query(query, (userrole,))
...
# Get all roles associated with a question by qid
@app.route('/api/getQuestionRoles/<int:qid>', methods=["GET"])
def get_question_roles(qid):
    with Postgres() as db:
        postgres_select_query = """
        SELECT userrole 
        FROM questionRoleMapping
        WHERE qid = %s
        """
        filter_values = (qid,)
        results = db.execute_select_query(postgres_select_query, filter_values)
    return jsonify({'result': results})
...
def delete_question(qid):
    try:
        with Postgres() as conn:
            # Start by deleting the role mappings associated with the question
            postgres_delete_roles_query = """
            DELETE FROM questionRoleMapping
            WHERE qid = %s
            """
            conn.execute_update_query(postgres_delete_roles_query, (qid,))
            
            # Delete the question itself
            postgres_delete_question_query = """
            DELETE FROM Questions
            WHERE qid = %s
            """
            result = conn.execute_update_query(postgres_delete_question_query, (qid,))
...
@app.route('/api/getAnswerCounts', methods=["GET"])
def getAnswerCounts():
    with Postgres() as answer:
        postgres_select_query = """ 
        SELECT q.qid, o.oid, COUNT(a.oid) as answer_count FROM questions q
        CROSS JOIN public.options o
        LEFT JOIN answers a ON q.qid = a.qid AND a.oid = o.oid
        GROUP BY q.qid, o.oid
        ORDER BY q.qid, o.oid ASC;
        """
        filter_values = ()
        results = answer.execute_select_query(postgres_select_query, (filter_values,))
    return jsonify({'result': results})
